...
* `!model <no>`
  ...
  * `data/models.yaml` が一覧のソースです。説明欄には、100万トークン当たりの input / cached input / output の料金目安も記載しています。残念ながら、Codexのexec機能にはモデル名一覧を取得する機能がないため、機能が提供されない限り、一覧表の更新は手動で行う必要があります。
...
外部クライアントの更新はCodexターンの完了を確認してから、ユーザー入力と最終回答のみを同期します。
